...
export default function P2P() {
  ...
  React.useEffect(() => {
    axios.get("http://localhost:3001/transactions/p2p-recipients", { withCredentials: true })
    ...
    try {
      ...
      if (result && result.message) {
        if (result.message === "Transfer successful") {
          ...
          toast({ title: "Unexpected Error", description: result.message, variant: "destructive" });
        }
      }
    } catch (error: any) {
      toast({
