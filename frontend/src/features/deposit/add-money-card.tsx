...
export default function AddMoneyCard({ refresh }: { refresh: () => void }) {
  ...
  const handleAddMoney = async (e: React.FormEvent) => {
    ...
    try {
      // The backend creates the order AND records the pending transaction
      // (amount + user) in one call. The client no longer authors that record.
      const order = await depositApi.createOrder(paisaAmount);
      ...
      const options = {
        ...
        handler: async (response: any) => {
          try {
            // Only the Razorpay-signed fields are sent. The webhook derives the
            // amount and user from the order it already stored.
            await depositApi.capture({
      ...
      razorpay.on("payment.failed", (response: any) => {
        ...
        // Let the backend mark our own pending order as failed (by order id).
        depositApi.fail(order.orderId);
      });
