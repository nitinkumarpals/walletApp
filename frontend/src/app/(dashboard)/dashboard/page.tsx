...
export default async function DashboardPage() {
  const balanceData = await serverApi.balance();
  ...
  const analyticsData = analytics
    ? {
        dailyTotals: analytics.dailyTotals.map((d) => ({
          date: d.date,
          inflow: d.inflowMinor / 100,
          outflow: d.outflowMinor / 100,
        })),
        totalIn: analytics.totalInMinor / 100,
        totalOut: analytics.totalOutMinor / 100,
        net: analytics.netMinor / 100,
      }
    : { dailyTotals: [], totalIn: 0, totalOut: 0, net: 0 };

  const statement = (await serverApi.statement()) ?? { transfers: [], deposits: [] };
  ...
  statement.transfers.forEach((t) => {
    ...
    if (isOut) totalSpent += t.amountMinor / 100;
        ...
        <div className="md:col-span-2">
          <BentoTile label="[02] quick" className="h-full">
            <div className="flex flex-col gap-2.5 mt-1">
              <Link href="/send" className="btn-lime justify-between w-full">
                <span className="flex items-center gap-2"><Send className="h-4 w-4" /> send money</span>
                <ArrowUpRight className="h-4 w-4" />
              </Link>
              <Link href="/add-money" className="btn-ghost justify-between w-full">
