...
export default async function SettingsPage() {
  ...
  const paymentHistory = (await serverApi.depositHistory()) ?? [];
          ...
          <div className="mt-2 divide-y divide-border">
            {paymentHistory.length === 0 ? (
              ...
              paymentHistory.map((item) => (
                <div key={item.method} className="flex items-center justify-between py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="mono text-sm text-foreground">{item.method}</div>
                      <div className="label-mono">
                        {item.completedCount} successful · {inr(item.totalAmountMinor)} topped up
                      </div>
                    </div>
                  </div>
